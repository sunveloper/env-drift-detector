...
def test_filename_claim_works_without_a_matching_suffix():
    registry = Registry([_YamlExtractor()])
    assert registry.handles("src/main/resources/application.yaml") is True
...
def test_register_adds_an_extractor_without_touching_the_defaults():
    ...
    registry.register(_YamlExtractor())

    assert registry.handles("application.yml") is True
    assert default_registry.handles("application.yml") is False  # defaults untouched
...
def test_scan_file_honours_a_custom_registry(tmp_path: Path):
    ...
    assert scan_file(target, tmp_path) == []  # default registry ignores .yml
    usages = scan_file(target, tmp_path, Registry([_YamlExtractor()]))
    assert [u.name for u in usages] == ["FROM_YAML"]
...
def test_iter_source_files_follows_the_registry(tmp_path: Path):
    ...
    registry = Registry([PythonExtractor(), _YamlExtractor()])
    found = sorted(p.name for p in iter_source_files(tmp_path, registry=registry))
    assert found == ["app.py", "config.yml"]
